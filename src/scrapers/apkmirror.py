...
class APKMirrorScraper(BaseScraper):
    ...
    def download(self, url: str, version: str, dest: Path, arch: str, dpi: str) -> DownloadResult:
        ...
        soup_final = parse_html(resp)
        a_tag = soup_final.select_one("span > a[rel=nofollow]")
    ...
    def _search(self, rows: list, dpi: str, arch: str, bundle_type: str) -> str:
        ...
        for row in reversed(rows):
            ...
            if len(span_texts) < 4:
                continue
            if span_texts[2] == bundle_type and span_texts[3] in appdpi and span_texts[1] in apparch:
                return href
        return ""
